...
```mermaid
flowchart TB
  subgraph open ["YOUR MACHINE · APACHE 2.0"]
    direction TB
    repo[Your repository] --> cli[codex-security CLI]
  end

  subgraph closed ["OPENAI · GATED"]
    model[gpt-5.6-sol reasoning]
  end

  cli -- your code --> model
  model -- verdicts --> out[Findings + source excerpts]
```
