...
```mermaid
flowchart TB
  subgraph abierto ["TU MAQUINA · APACHE 2.0"]
    direction TB
    repo[Tu repositorio] --> cli[CLI codex-security]
  end

  subgraph cerrado ["OPENAI · RESTRINGIDO"]
    model[Razonamiento gpt-5.6-sol]
  end

  cli -- tu codigo --> model
  model -- veredictos --> out[Hallazgos + codigo fuente]
```
