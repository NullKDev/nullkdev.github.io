...
```mermaid
flowchart TB
  subgraph cerrado ["OPENAI · RESTRINGIDO, NO AUDITABLE"]
    gate{Puerta de acceso} -- aprobado --> model[Razonamiento gpt-5.6-sol]
  end

  subgraph abierto ["TU MAQUINA · APACHE 2.0, AUDITABLE"]
    repo[Repositorio en disco] --> cli[CLI codex-security]
    out[Hallazgos y fragmentos de codigo]
  end

  cli -- tu codigo --> gate
  model -- veredictos --> out
```
...
<BenchmarkBars
  label="Escáner"
  metric="Precisión"
  scale="1.0 = todo hallazgo es real · 0.0 = todo hallazgo es ruido"
  caption="Precisión en RealVuln, 26 repositorios Python vulnerables, 796 hallazgos etiquetados a mano."
  rows={[
    {
      label: 'Claude Sonnet 4.6',
      note: 'LLM de propósito general',
      value: 0.785,
    },
    { label: 'Gemini 3.1 Pro', note: 'LLM de propósito general', value: 0.774 },
    { label: 'SonarQube', note: 'SAST basado en reglas', value: 0.611 },
    { label: 'SecLab Agent', note: 'Especializado en seguridad', value: 0.605 },
    { label: 'Snyk', note: 'SAST basado en reglas', value: 0.282 },
    {
      label: 'Semgrep',
      note: 'SAST basado en reglas — el titular actual',
      value: 0.205,
      emphasis: true,
    },
  ]}
...
<Callout type="warn" title="La salida del escaneo es un artefacto sensible">
  Una advertencia que la documentación plantea y que la cobertura pasó por alto:
  los resultados contienen **fragmentos de código fuente y detalles de las
  vulnerabilidades**. Si apuntás `--output-dir` dentro del repositorio, tarde o
  temprano vas a commitear un archivo que describe exactamente cómo explotar tu
  propio producto. Mantenelo fuera del árbol y con una política de retención —
  el cableado está en el [complemento sobre integración con
  CI](/es/notes/codex-security-brecha-precision/integracion-ci/).
</Callout>
