...
imageAlt: Remote Compose banner — a serialized Compose document rendered by an on-device player.
links:
  - label: Official release notes
    href: https://developer.android.com/jetpack/androidx/releases/compose-remote
    kind: publication
references: []
evidence: []
documents:
...
series:
  ...
  order: 1
citations:
  - title: Compose Remote release notes
...
In the setup walkthrough that follows, we build a practical example: creating a simple Remote Compose document on the creation side, serializing it, and rendering it inside an Android app using `remote-player-view`.
