...
function loadPretext(): Promise<PretextMod | null> {
  if (!pretextPromise) {
    // Bundled from node_modules, not fetched from a CDN at runtime: a
    // third-party origin serving executable code into a reader's browser is a
    // supply-chain dependency this archive does not need to take on, and it
    // contradicts the local-first claim the rest of the site makes.
    pretextPromise = import('@chenglou/pretext')
      .then((mod) => mod as unknown as PretextMod)
      .catch(() => null)
  }
...
export default function MatteflowDemo() {
  ...
  return (
    <div className="not-prose border-border bg-card overflow-hidden rounded-xl border shadow-sm">
      {/* Not media for the viewer: this element only decodes frames that are
          read into the canvas below. It is never shown and has no audio, so a
          captions track would be theatre — it is hidden from the a11y tree
          instead, which is also what satisfies the captions rule without
          suppressing it. `muted` + `playsInline` are what let it autoplay. */}
      <video
        ref={videoRef}
        className="hidden"
        aria-hidden="true"
